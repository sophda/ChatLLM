...
int testmodel() {
    ...
    std::string config_path = "/home/sophda/llmprojects/ChatRobot/qwenmodel/model/config.json";
    ...
    while (true) {
        ...
        std::cout << assistant_str << std::endl;
...
// int main() {
//     Model model;
//     std::string model_path = "/home/sophda/llmprojects/ChatRobot/qwenmodel/model/config.json";
//     model.init(model_path);
//     std::string input = "你好，你叫什么名字？";
//     std::string output;
...
void initModel() {
    ...
    model->init("/storage/emulated/0/nahidallm/config.json");
...
Java_com_example_chatllm_NativeInterface_initModel(JNIEnv *env, jclass clazz) {
...
Java_com_example_chatllm_NativeInterface_getreply(JNIEnv *env, jclass clazz, jstring message) {
    // TODO: implement getreply()
    const char *input_char = env->GetStringUTFChars(message,NULL);
    std::string input_str = input_char;
    std::string output ;
    model->predict(input_str, output);
